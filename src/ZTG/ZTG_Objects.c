...
bool ztg_button(Button b, char * label, int padding_x, int padding_y, int label_color){
    bool is_hover = ztg_collision_point_inside_rect(b.bounds, window.mousePos);
    bool is_pressed = ztg_is_mouse_button_pressed(MOUSE_LEFT_BUTTON);

    int curr_color = is_hover ? is_pressed ? b.color_pressed : b.color_hover : b.color_base;
